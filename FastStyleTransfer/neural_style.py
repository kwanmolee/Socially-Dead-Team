...
def realtime_stylize(args):
    ...
    # create 3 models
    models = ["./saved_models/mosaic.pth", "./saved_models/candy.pth", "./saved_models/rain_princess.pth"]
    style_models = list()
    for model in models:
        style_models.append(TransformerNet())
        style_models[-1].load_state_dict(torch.load(model))
...
def main():
    ...
    realtime_arg_parser = subparsers.add_parser("realtime", help="parser for realtime-model arguments")
    realtime_arg_parser.add_argument("--cuda", type=int, required=True,
                                 help="set it to 1 for running on GPU, 0 for CPU")
